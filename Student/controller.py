...
def insert_in_student():
    if isinstance(check_user(),bool):
        ...
        if isinstance(validity, bool):
            ...
            return jsonify(db.give_response(data=[], message=message, start_time=start_time))
        else:
            return jsonify(error_handler.generate_error_response(validity))
    else:
        return jsonify(error_handler.generate_error_response(check_user()))
...
def student_login():
    ...
    if isinstance(validity, bool):
        ...
        if give_hash(input_data['password']) == store_password[0][0]:
            ...
            return jsonify(db.give_response(data=[], message="login successfull !!!", start_time=start_time, token=access_token))
        else:
            return jsonify(db.give_response(data=[], message="wrong username or password !!", start_time=start_time))
    else:
        return jsonify(error_handler.generate_error_response(validity))
